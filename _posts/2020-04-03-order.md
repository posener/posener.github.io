...
[explicitly chose](https://golang.org/doc/faq#overloading) not permit operator overloading
...
In Go, the different types can be either comparable, ordered, both comparable and ordered, or none.
...
Using the three-way comparison function might be harder to understand: reading `cmp(a, b) < 0` not
translated immediately as `a < b`. One way to remember how to translate the reading of such
...
readable: for example, reading `a.After(b)` is very clear. However, in some cases it can be
overwhelming: for example, checking if `a >= b` is done by two function calls:
`a.After(b) || a.Equal(b)`.
...
*   `int32(1) != int64(1)`, or given `type s string`, `"foo" != s("foo")`. Nevertheless the value is
    identical, comparing different types is compile time error. In this case, the developer should
    convert the types in a **safe way** before comparing them, for example convert the `int32` to
    `int64` and not the other way around.
...
*   Given `a, b := 0, 0`, `&a != &b`. Even though the content is identical, When comparing pointers,
    the address is compared and the address is different.

This is great for equality comparison, which in many programming use cases care about the compared
type. However, when defining order is required, the type is less important, different types can be
ordered as long as the conversion is safe. For example, for a defined order over string types
(`func cmp(string, string) int { ... }`), we should be able to sort a slice of `*string`
(`var s []*string`). In this case we would like to compare the value that is pointed by each element
in the slice (call for any `i`, `j`: `cmp(*s[i], *s[j])`).
