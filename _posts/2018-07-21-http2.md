...
But currently the external package [github.com/veqryn/h2c](https://github.com/veqryn/h2c) can be used.
...
But I wander if there could be a nicer way to add this capability.
...
The server logs will show what we expect:
```
Got connection: HTTP/2.0
Handling 1st
Got connection: HTTP/2.0
...
The server copy everything from the request body reader to a "flush writer" that writes
...
func main() {
    ...
    // Create an `http.Request` and set it's body as the reader part of the pipe - after sending
...
The down side of this example is the API - the standard library provides us with powerful tools,
