...
But here, the response starts to stream before the request stream is finished.
After each time we send data through the pipe to the request, data is returned from the server in the response
stream.
